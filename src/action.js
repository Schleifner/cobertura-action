...
async function action(payload) {
  ...
  for (const report of reports) {
    ...
    for (const file of report.files.filter(
      (file) => changedFiles == null || changedFiles.includes(file.filename)
    )) {
      const fileLines = Math.floor(file.line);
      const fileBranch = Math.floor(file.branch);

      if (
        fileLines < minimumLineCoverage ||
        fileBranch < minimumBranchCoverage
      ) {
        core.setFailed("gkb 1");
